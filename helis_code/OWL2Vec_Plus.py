...
class InclusionEvaluator(Evaluator):
    ...
    def evaluate(self, model, eva_samples):
        ...
        for k, sample in enumerate(eva_samples):
            ...
            embedding_P = model.predict_proba(X)
            P = embedding_P[:, 1]
            ...
            if rank_non > rank:
                infer = [x.split(('#'))[1] for x in sorted_classes[:rank]]
                unique_class = list(set(sorted_classes_non[:rank_non]).symmetric_difference(set(sorted_classes[:rank])))                
                print('Predicted inferable class(es):', [x.split(('#'))[1] for x in unique_class], " with rank:", [sorted_classes_non.index(x)+1 for x in unique_class])
                print('True class:', infer[-1])
                print(f"Individual: {individual.split(('#'))[1]} with rank:[{rank_non}(no-checking), {rank}(checked)]")
                if len(unique_class) > 0:
                    for x in unique_class:
                        if x in ind_infer_classes[individual]:
                            our_def = our_def+1
                print('\n')
            MRR_sum += 1.0 / rank
        ...
        print('Inferable for non infer: ',str(our_def))
