...
class InclusionEvaluator(Evaluator):
    ...
    def evaluate(self, model, eva_samples):
        ...
        for k, sample in enumerate(eva_samples):
            ...
            if rank_non > rank:
                infer = [x for x in sorted_classes[:rank]]
                unique_class = list(set(sorted_classes_non[:rank_non]).symmetric_difference(set(sorted_classes[:rank])))                
                print('Predicted ancestor class(es):', [x for x in unique_class], " with rank:", [sorted_classes_non.index(x)+1 for x in unique_class])
                print('True parent class:', infer[-1])
                print(f"The input class: {sub} with rank:[{rank_non}(no-checking), {rank}(checked)]")
                print('\n')
            MRR_sum += 1.0 / rank
